<script lang="ts">
	...
	let { song, position, source = '', showPreview = false, ytConfigured = false, initialCached = null, resolvedTrack = null, onPlay }: Props = $props();
...
<div class="flex items-center gap-3 p-2 rounded-lg hover:bg-base-200 transition-colors group">
	{#if canPlay}
		<button onclick={onPlay} class="w-6 flex-shrink-0 flex items-center justify-center cursor-pointer" aria-label="Play {song.title}">
	...
	{:else}
		<a href={albumHref(song.release_group_mbid ?? '')} class="w-6 flex-shrink-0 flex items-center justify-center">
			<span class="group-hover:hidden text-sm text-base-content/50">{position}</span>
			<span class="hidden group-hover:block">
				<Play class="w-4 h-4 mx-auto fill-current" />
			</span>
	...
	<a href={albumHref(song.release_group_mbid ?? '')} class="flex items-center gap-3 flex-1 min-w-0 cursor-pointer">
		<div class="w-12 h-12 flex-shrink-0">
			<AlbumImage mbid={song.release_group_mbid ?? ''} alt={song.release_name || ''} size="full" className="w-12 h-12 rounded" />
		</div>
...
{:else}
<div
	class="flex items-center gap-3 p-2 rounded-lg transition-colors group {isLastfmNoAlbum ? 'opacity-75' : ''}"
>
	{#if canPlay}
		<button onclick={onPlay} class="w-6 flex-shrink-0 flex items-center justify-center cursor-pointer" aria-label="Play {song.title}">
			<span class="group-hover:hidden text-sm text-base-content/50">{position}</span>
