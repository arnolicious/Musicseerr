<script lang="ts">
	...
	import {
		ChevronDown, X, RotateCcw, Trash2, Clock, Download, Loader,
		CheckCircle2, XCircle, AlertTriangle, Ban, Pause, Music, Upload
	} from 'lucide-svelte';
	...
	const statusConfig = $derived.by((): StatusConfig => {
		switch (item.status) {
			case 'pending': return { badgeClass: 'badge-warning', label: 'Searching', icon: Clock, glow: 'shadow-warning/20' };
			case 'queued': return { badgeClass: 'badge-warning', label: 'Queued', icon: Clock };
			case 'downloading': return { badgeClass: 'badge-info', label: 'Downloading', icon: Download, glow: 'shadow-info/20' };
			case 'importing': return { badgeClass: 'badge-info', label: 'Importing', icon: Upload };
			case 'paused': return { badgeClass: 'badge-warning', label: 'Paused', icon: Pause };
			case 'downloadClientUnavailable': return { badgeClass: 'badge-error', label: 'Client Unavailable', icon: AlertTriangle };
			case 'importFailed': return { badgeClass: 'badge-error', label: 'Import Failed', icon: XCircle };
			case 'importBlocked': return { badgeClass: 'badge-warning', label: 'Import Blocked', icon: AlertTriangle };
			case 'incomplete': return { badgeClass: 'badge-warning', label: 'Incomplete', icon: AlertTriangle };
			case 'imported': return { badgeClass: 'badge-success', label: 'Imported', icon: CheckCircle2 };
			case 'failed': return { badgeClass: 'badge-error', label: 'Failed', icon: XCircle };
			case 'cancelled': return { badgeClass: 'badge-ghost', label: 'Cancelled', icon: Ban };
			default: return { badgeClass: 'badge-ghost', label: item.status, icon: Music };
		}
	});

	let showStatusDetails = $state(false);
	const isFailedState = $derived(
		item.status === 'importFailed' || item.status === 'importBlocked' || item.status === 'failed' || item.status === 'downloadClientUnavailable'
	);
	...
	const hasProgress = $derived(
		isActive && (activeItem.status === 'downloading' || activeItem.status === 'importing' || activeItem.status === 'paused')
	);
...
>
	...
	<div class="relative z-10 flex items-center gap-3 sm:gap-4 p-3 sm:p-4 pointer-events-none">
		<div class="w-14 h-14 sm:w-[72px] sm:h-[72px] flex-shrink-0 rounded-lg overflow-hidden relative">
			<AlbumImage
		...
		<div class="flex-1 min-w-0">
			...
			{:else}
				<p class="text-xs sm:text-sm text-base-content/70 line-clamp-1 text-left">{item.artist_name}</p>
			{/if}
		...
		<div class="flex flex-col items-end gap-1.5 flex-shrink-0 pointer-events-auto">
			...
			{#if hasProgress}
				<div class="flex flex-col items-end gap-1 w-28 sm:w-44" role="status" aria-label="Download progress">
					<div class="flex items-center gap-2 w-full">
						...
						<span class="text-xs text-base-content/70 min-w-[3ch] text-right tabular-nums font-medium">
			...
			{:else if isActive && isFailedState && activeItem.error_message}
				<span class="text-xs text-error/80 max-w-44 text-right line-clamp-2">{activeItem.error_message}</span>
			...
			<div class="flex gap-1 items-center">
				{#if isActive && hasStatusMessages}
					...
					>
						<ChevronDown class="h-3.5 w-3.5 transition-transform duration-200 {showStatusDetails ? 'rotate-180' : ''}" />
					</button>
				{/if}
				{#if isActive}
					{#if confirmingCancel}
				...
				{:else}
					{#if historyItem.status === 'failed' || historyItem.status === 'cancelled' || historyItem.status === 'incomplete'}
						<button class="btn btn-xs btn-primary btn-ghost" onclick={handleRetry} title="Retry request">
							<RotateCcw class="h-3.5 w-3.5" />
						</button>
					{/if}
					{#if historyItem.status === 'imported' && historyItem.in_library}
						<button class="btn btn-xs btn-ghost text-base-content/40 hover:text-error" onclick={handleRemoveFromLibrary} title="Remove from library">
							<Trash2 class="h-3.5 w-3.5" />
						</button>
					{/if}
					<button class="btn btn-xs btn-ghost text-base-content/30 hover:text-base-content/70" onclick={handleClear} title="Clear from history">
						<X class="h-3.5 w-3.5" />
					</button>
				{/if}
			</div>
	...
	{#if showStatusDetails && hasStatusMessages}
		<div class="px-3 sm:px-4 pb-3 sm:pb-4 -mt-1 relative z-10">
			<div class="bg-base-100/50 border border-error/10 rounded-lg p-3 text-xs max-h-48 overflow-y-auto">
